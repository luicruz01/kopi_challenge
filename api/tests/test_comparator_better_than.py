...
class TestComparatorBetterThan:
    ...
    def test_coke_vs_pepsi_better_than(self):
        ...
        response = engine.generate_response(
            "general", "opposing", "explain why coke is better than pepsi", conversation_history, "en"
        )
        ...
        axis_terms = ["more direct", "requires additional context", "easier to get started", "demands initial practice", "prioritizes the essential", "coherent long-term decisions"]
    ...
    def test_spanish_mejor_que_pattern(self):
        ...
        assert any(term in response_lower for term in ["coca", "cola", "pepsi"]), f"Should mention brands: {response}"
        
        # Should contain Spanish axis arguments
        spanish_axis_terms = ["decisiones coherentes", "soluciones ad hoc", "más fácil empezar", "se adapta mejor", "favorece"]
    ...
    def test_stance_opposite_logic(self):
        ...
        for user_message, user_pref, bot_pref in test_cases:
            ...
            assert bot_pref.lower() in response_lower, f"Should mention bot's side {bot_pref}: {response}"
            assert user_pref.lower() in response_lower, f"Should mention user's side {user_pref}: {response}"
    ...
    def test_no_unrelated_topic_drift(self):
        ...
        for user_message in test_cases:
            ...
            forbidden_terms = [
                "technology", "climate", "environment", "carbon", "emissions",
                "renewable", "education", "school", "spirituality", "meditation"
            ]
            
            forbidden_found = [term for term in forbidden_terms if term in response_lower]
            assert not forbidden_found, f"Should not mention unrelated topics {forbidden_found}: {response}"
            ...
            axis_terms = [
                ...
                "more direct", "requires additional context", "minimizes interruptions", "introduce micro-decisions"
            ]
            axis_found = any(term in response_lower for term in axis_terms)
    ...
    def test_structure_preservation(self):
        ...
        claim_found = "your core claim" in response_lower or "maintain the opposite" in response_lower
