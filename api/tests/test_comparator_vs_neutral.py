...
class TestComparatorVsNeutral:
    ...
    def test_vim_vs_emacs_neutral(self):
        ...
        axis_terms = [
            "immediate results", "reduces friction", "adds extra steps", "more intuitive", 
            "steep curve", "easier to get started", "demands initial investment",
            "coherent long-term decisions", "ad hoc solutions", "prioritizes the essential", 
            "scatters attention", "adapts better to changes", "structural rigidity"
        ]
    ...
    def test_correct_ab_ordering(self):
        ...
        substitution_working = ("pizza" in response_lower and "burgers" in response_lower and
                               "{{A}}" not in response and "{{B}}" not in response)
    ...
    def test_spanish_vs_patterns(self):
        ...
        spanish_axis_terms = [
            "decisiones coherentes", "soluciones ad hoc", "más fácil empezar", 
            "se adapta mejor", "favorece", "prioriza lo esencial", "dispersa la atención",
            "resultados inmediatos", "reduce fricción", "añade pasos extra"
        ]
    ...
    def test_no_research_evidence_phrasing(self):
        ...
        for user_message in test_cases:
            ...
            forbidden_phrases = [
                "research shows", "studies demonstrate", "evidence indicates",
                "data shows", "research confirms", "scientific evidence"
            ]
            
            forbidden_found = [phrase for phrase in forbidden_phrases if phrase in response_lower]
            assert not forbidden_found, f"Should avoid research/evidence phrasing {forbidden_found}: {response}"
            
            # Should use the new openings instead
            acceptable_openings = [
                "i can accept", "worth considering", "test what happens"
            ]
    ...
    def test_multilingual_consistency(self):
        ...
        en_response = engine.generate_response(
            "general", "opposing", "red vs blue", [], "en"
        )
        
        es_response = engine.generate_response(
            "general", "opposing", "rojo vs azul", [], "es"
        )
        ...
        en_axis = any(term in en_lower for term in [
            "coherent long-term decisions", "ad hoc solutions", "more intuitive", 
            "steep curve", "easier to get started", "demands initial practice"
        ])
        es_axis = any(term in es_lower for term in [
            "decisiones coherentes", "soluciones ad hoc", "más intuitivo", 
            "curva empinada", "más fácil empezar", "exige práctica"
        ])
    ...
    def test_closing_with_substitution(self):
        ...
        response_lower = response.lower()
        assert "books" in response_lower or "movies" in response_lower, "Should contain actual terms"
