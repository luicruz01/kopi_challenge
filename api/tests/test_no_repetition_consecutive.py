...
class TestNoRepetitionConsecutive:
    ...
    def test_no_consecutive_opening_repetition(self):
        ...
        for i in range(3):
            ...
            user_turn = Turn(role="user", message=user_message, sequence=len(conversation_history) + 1)
        ...
        for i in range(len(openings) - 1):
            assert openings[i] != openings[i + 1], f"Consecutive openings should differ: {openings[i]} vs {openings[i + 1]}"
    ...
    def test_no_consecutive_closing_repetition(self):
        ...
        for response in responses:
            ...
            if len(sentences) > 1:
                ...
                closing = sentences[-2].strip().lower() if sentences[-1].strip() == "" else sentences[-1].strip().lower()
    ...
    def test_no_consecutive_axis_repetition(self):
        ...
        axis_keywords = ["simplicity", "consistency", "accessibility", "essential", "practice", "interruptions", "results"]
        ...
        assert len(all_axes_used) > 1, f"Should use different axes across responses: {response_axes}"
    ...
    def test_deterministic_but_varied_responses(self):
        ...
        response2 = engine.generate_response(
            "general", "opposing", "tea vs coffee", conversation_history3, "en"  # Reversed order
        )
    ...
    def test_rotation_with_conversation_history(self):
        ...
        for response in [response1, response2, response3]:
            response_lower = response.lower()
            assert "python" in response_lower and "java" in response_lower, f"Should mention both languages: {response}"
    ...
    def test_spanish_no_consecutive_repetition(self):
        ...
        assert len(unique_openings) > 1, f"Should have variety in Spanish openings: {opening_matches}"
    ...
    def test_mixed_patterns_no_repetition(self):
        ...
        unique_responses = set(responses)
        assert len(unique_responses) > 1, f"Should have variety across different patterns: got {len(unique_responses)} unique responses"
